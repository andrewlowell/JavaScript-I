...
for (let c of inventory) {
  if (c.id === 33) {
    console.log(c.car_year + ' ' + c.car_model + ' ' + c.car_make);
  }
}
...
for (let c of inventory) {
  if (c.id === 50) {
    console.log(c.car_model + ' ' + c.car_make);
  }
}
...
for (let c of inventory) {
  ...
  if (c.car_make == 'Audi' || c.car_make == 'BMW') {
    BMWAndAudi.push(c);
  }
}
